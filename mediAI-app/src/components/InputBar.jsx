...
export default function InputBar({
  ...
  const handleFile = useCallback(
    (selected) => {
      ...
      // Only take the number of files allowed
      const limitedSelection = Array.from(selected).slice(0, remainingSlots);

      const mapped = limitedSelection.map((f) => ({
        file: f,
        name: f.name,
    ...
    <div className="relative">
        ...
        <div className="mb-3 bg-[#070d1a]/95 backdrop-blur-xl border border-white/8 rounded-2xl p-6 flex flex-col items-center gap-5 animate-message-in">
          ...
          {text && (
            <div className="w-full max-w-sm">
              <div className="text-xs text-slate-600 mb-1.5 uppercase tracking-wider">
                Transcript
              </div>
              <div
                className={`text-sm text-slate-200 text-center rounded-xl px-4 py-2.5 border transition-colors duration-300 ${
                  isListening
          ...
          <div className="flex gap-3 items-center">
            <button
              ...
              className={`cursor-pointer px-5 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                isListening
      ...
      {files.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2 px-2">
          ...
          {files.length >= 4 && (
            <div className="text-[10px] text-amber-500 flex items-center px-1">
              Max 4 files reached
            </div>
          )}
        </div>
      ...
      <div
        ...
        onDragOver={(e) => {
          e.preventDefault();
          if (files.length < 4) setDragOver(true);
        }}
      ...
      >
        <div className="flex items-center gap-1 pb-1">
          <button
            ...
            className={`cursor-pointer w-8 h-8 flex items-center justify-center rounded-xl transition-all ${
              files.length >= 4
                ? "text-slate-700 cursor-not-allowed"
                : "text-slate-500 hover:text-slate-300 hover:bg-white/5"
            }`}
            title={files.length >= 4 ? "Limit reached" : "Upload image or PDF"}
          >
            <svg
              className="w-4 h-4"
              fill="none"
