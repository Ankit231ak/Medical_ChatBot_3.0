...
export default function WelcomeScreen({ onSuggest, assistantState = 'normal' }) {
  return (
    <div className="flex flex-col items-center justify-start flex-1 px-4 pt-8 pb-4 sm:pt-16 sm:pb-8 animate-fadeIn min-h-full">
      {/* Animated logo */}
      {/* Animated Video Avatar */}
      <div className="relative mb-3 sm:mb-6 flex justify-center mt-1 sm:mt-1 overflow-visible">
        {/* Glow behind the avatar - adjusted for a softer, more premium look */}
        <div className="absolute left-1/2 top-1/2 w-56 h-56 sm:w-[20rem] sm:h-[20rem] -translate-x-1/2 -translate-y-[46%] rounded-full bg-cyan-500/20 blur-[45px] sm:blur-[65px] animate-blob pointer-events-none" />
        <div className="absolute left-1/2 top-1/2 w-48 h-48 sm:w-[18rem] sm:h-[18rem] -translate-x-1/2 -translate-y-[46%] rounded-full bg-blue-500/10 blur-3xl animate-spin-slow pointer-events-none opacity-60" />

        <AssistantAvatar
          state={assistantState}
          hoverToListen
          className="w-48 h-48 sm:w-64 sm:h-64 drop-shadow-[0_0_24px_rgba(34,211,238,0.4)] sm:drop-shadow-[0_0_34px_rgba(34,211,238,0.45)]"
        />
      </div>
