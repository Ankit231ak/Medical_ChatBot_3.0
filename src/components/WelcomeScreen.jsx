...
export default function WelcomeScreen({ onSuggest, assistantState = 'normal' }) {
  return (
    <div className="flex flex-col items-center justify-center flex-1 px-4 py-8 animate-fadeIn">
      {/* Animated logo */}
      {/* Animated Video Avatar */}
      <div className="relative mb-8 flex justify-center mt-4">
        {/* Glow behind the avatar */}
        <div className="absolute inset-0 w-80 h-80 mx-auto rounded-full bg-cyan-600/30 blur-[60px] animate-blob pointer-events-none" />
        <div className="absolute inset-0 w-72 h-72 mx-auto rounded-full bg-blue-600/20 blur-3xl animate-spin-slow pointer-events-none" />

        <AssistantAvatar
          state={assistantState}
          className="w-72 h-72 drop-shadow-[0_0_40px_rgba(34,211,238,0.6)]"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2 justify-center mb-8">
        {[
          ...
          <div key={f.text} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs text-slate-300">
            <span>{f.icon}</span>
            {f.text}
          </div>
        ))}
      ...
      {/* Disclaimer */}
      <div className="mt-10 flex items-start gap-2 bg-amber-500/5 border border-amber-500/15 rounded-xl px-4 py-3 max-w-md">
